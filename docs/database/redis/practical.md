...
```
例如，执行set name jack，则脚本是这样：
```lua
# 执行 set name jack
redis.call('set', 'name', 'jack')
```
例如，先执行set name Rose，再执行get name，则脚本如下：
```lua
# 先执行 set name jack
...
![image.png](https://cdn.nlark.com/yuque/0/2023/png/32600948/1682816689827-1f43bd6c-2fdc-4efb-89b1-743f372947d0.png#averageHue=%23031f32&clientId=u9fd07684-81ff-4&from=paste&height=163&id=uabdb7f4b&originHeight=163&originWidth=714&originalType=binary&ratio=1&rotation=0&showTitle=false&size=16420&status=done&style=none&taskId=u4017231c-98f1-4a00-b652-482579ed499&title=&width=714)
例如，我们要执行 redis.call('set', 'name', 'jack') 这个脚本，语法如下：
![image.png](https://cdn.nlark.com/yuque/0/2023/png/32600948/1682816736882-fb97b91e-7b08-4eaf-b224-a82277aca1e6.png#averageHue=%23022134&clientId=u9fd07684-81ff-4&from=paste&height=113&id=u90c9058d&originHeight=113&originWidth=668&originalType=binary&ratio=1&rotation=0&showTitle=false&size=9701&status=done&style=none&taskId=u728b2e0f-8b63-477c-b161-0b3a20b42ad&title=&width=668)
如果脚本中的key、value不想写死，可以作为参数传递。key类型参数会放入KEYS数组，其它参数会放入ARGV数组，在脚本中可以从KEYS和ARGV数组获取这些参数：
![image.png](https://cdn.nlark.com/yuque/0/2023/png/32600948/1682816772827-70c12041-1af8-4041-b7dd-6338c3783f29.png#averageHue=%23122c3f&clientId=u9fd07684-81ff-4&from=paste&height=151&id=ud278d747&originHeight=151&originWidth=1129&originalType=binary&ratio=1&rotation=0&showTitle=false&size=81655&status=done&style=none&taskId=u2e9ff39f-187e-4e1d-9772-17552d4e3a4&title=&width=1129)
接下来回顾一下释放锁的逻辑，
释放锁的业务流程是这样的：
...
![image.png](https://cdn.nlark.com/yuque/0/2023/png/32600948/1682818213348-344546fe-28dd-401d-80fc-cd9355e3440a.png#averageHue=%23eff3eb&clientId=u9fd07684-81ff-4&from=paste&height=384&id=u650ec173&originHeight=384&originWidth=822&originalType=binary&ratio=1&rotation=0&showTitle=false&size=124278&status=done&style=none&taskId=ubff98678-d9f3-4137-9556-d1ed9657949&title=&width=822)
**Java代码**
```java
...
但是目前还剩下一个问题锁不住，如果当过期时间到了之后，我们可以给他续期一下，比如续个30s，就好像是网吧上网， 网费到了之后，然后说，来，网管，再给我来10块的，是不是后边的问题都不会发生了，那么续期问题怎么解决呢，可以依赖于我们接下来要学习redission啦
**测试逻辑：**
...
![image.png](https://cdn.nlark.com/yuque/0/2023/png/32600948/1682818622134-2108b57d-2c8e-41f7-bce6-9c9fa83bccde.png#averageHue=%23f9f8f8&clientId=u9fd07684-81ff-4&from=paste&height=621&id=u97a636cd&originHeight=621&originWidth=1541&originalType=binary&ratio=1&rotation=0&showTitle=false&size=180941&status=done&style=none&taskId=u1ec6e71f-659e-4890-9d91-4cb599b925a&title=&width=1541)
什么是Redission？
Redisson是一个在Redis的基础上实现的Java驻内存数据网格（In-Memory Data Grid）。它不仅提供了一系列的分布式的Java常用对象，还提供了许多分布式服务，其中就包含了各种分布式锁的实现。
Redission提供了分布式锁的多种多样的功能
![image.png](https://cdn.nlark.com/yuque/0/2023/png/32600948/1682819023290-2784622e-5698-4d1a-8bcd-b76b58e77bfc.png#averageHue=%23fdfdfd&clientId=u9fd07684-81ff-4&from=paste&height=402&id=u2cb423db&originHeight=402&originWidth=920&originalType=binary&ratio=1&rotation=0&showTitle=false&size=43971&status=done&style=none&taskId=u92d28e74-f2e4-44ab-bffd-b2f2039b53b&title=&width=920)
### 快速入门
引入依赖：
```java
<dependency>
...
public class RedissonConfig {
    ...
    public RedissonClient redissonClient() {
        ...
        config.useSingleServer().setAddress("redis://192.168.1.10:6379").setPassword("root");
        // 创建RedissonClient对象
...
在分布式锁中，他采用hash结构用来存储锁，其中大key表示表示这把锁是否存在，用小key表示当前这把锁被哪个线程持有，所以接下来分析一下这个lua表达式
这个地方一共有3个参数
**KEYS[1] ： 锁名称**
**ARGV[1]：  锁失效时间**
...
从前边的源码可以发现他会去判断当前这个方法的返回值是否为null，如果是null，则对应则前两个if对应的条件，退出抢锁逻辑，如果返回的不是null，即走了第三个分支，在源码处会进行while(true)的自旋抢锁。
```lua
...
```
![image.png](https://cdn.nlark.com/yuque/0/2023/png/32600948/1682819307911-5f842909-7d56-4589-a817-3e04906cea7f.png#averageHue=%23f1f3ee&clientId=u9fd07684-81ff-4&from=paste&height=720&id=uf8d21976&originHeight=720&originWidth=1655&originalType=binary&ratio=1&rotation=0&showTitle=false&size=378105&status=done&style=none&taskId=u16d44c24-dae1-4e14-993c-16a0ec1000b&title=&width=1655)
...
![image.png](https://cdn.nlark.com/yuque/0/2023/png/32600948/1682819331079-d4ad68da-c467-415a-9f09-4b57357d7138.png#averageHue=%23f7ebeb&clientId=u9fd07684-81ff-4&from=paste&height=536&id=u7ec15605&originHeight=536&originWidth=1121&originalType=binary&ratio=1&rotation=0&showTitle=false&size=108899&status=done&style=none&taskId=ucb5710fd-c634-4516-97a1-dacea0a812d&title=&width=1121)
为了解决这个问题，redission提出来了MutiLock锁，使用这把锁咱们就不使用主从了，每个节点的地位都是一样的， 这把锁加锁的逻辑需要写入到每一个主丛节点上，只有所有的服务器都写入成功，此时才是加锁成功，假设现在某个节点挂了，那么他去获得锁的时候，只要有一个节点拿不到，都不能算是加锁成功，就保证了加锁的可靠性。
![image.png](https://cdn.nlark.com/yuque/0/2023/png/32600948/1682819341686-9b614b65-4eda-48bc-a500-3c8e27289655.png#averageHue=%23f2dede&clientId=u9fd07684-81ff-4&from=paste&height=579&id=ub48863cb&originHeight=579&originWidth=1591&originalType=binary&ratio=1&rotation=0&showTitle=false&size=215005&status=done&style=none&taskId=ud0c62ba8-2aa7-45c5-aecd-54ac8e641b8&title=&width=1591)
那么MutiLock 加锁原理是什么呢？笔者画了一幅图来说明
当我们去设置了多个锁时，redission会将多个锁添加到一个集合中，然后用while循环去不停去尝试拿锁，但是会有一个总共的加锁时间，这个时间是用需要加锁的个数 * 1500ms ，假设有3个锁，那么时间就是4500ms，假设在这4500ms内，所有的锁都加锁成功， 那么此时才算是加锁成功，如果在4500ms有线程加锁失败，则会再次去进行重试.
![image.png](https://cdn.nlark.com/yuque/0/2023/png/32600948/1682819353509-ada1e77a-4046-4a99-91e4-08c279858330.png#averageHue=%23f8f8f8&clientId=u9fd07684-81ff-4&from=paste&height=428&id=u3bbd195c&originHeight=428&originWidth=1411&originalType=binary&ratio=1&rotation=0&showTitle=false&size=52097&status=done&style=none&taskId=uaf87d278-a434-4f34-babc-1e2e71a15fb&title=&width=1411)
## 秒杀优化
### 秒杀优化-异步秒杀思路
回顾一下下单流程
...
在这六步操作中，又有很多操作是要去操作数据库的，而且还是一个线程串行执行， 这样就会导致程序执行很慢，所以需要异步程序执行，那么如何加速呢？
...
![image.png](https://cdn.nlark.com/yuque/0/2023/png/32600948/1682819477006-ad55357e-edc4-4c8f-9ebc-d9a315f71719.png#averageHue=%23fcfcfc&clientId=u9fd07684-81ff-4&from=paste&height=757&id=uef19ffc8&originHeight=757&originWidth=1536&originalType=binary&ratio=1&rotation=0&showTitle=false&size=160151&status=done&style=none&taskId=uc6f49e5c-70dd-45da-b212-3977d6cc3c6&title=&width=1536)
优化方案：我们将耗时比较短的逻辑判断放入到redis中，比如是否库存足够，比如是否一人一单，这样的操作，只要这种逻辑可以完成，就意味着我们是一定可以下单完成的，我们只需要进行快速的逻辑判断，根本就不用等下单逻辑走完，我们直接给用户返回成功， 再在后台开一个线程，后台线程慢慢的去执行queue里边的消息，这样程序不就超级快了吗？而且也不用担心线程池消耗殆尽的问题，因为这里我们的程序中并没有手动使用任何线程池，当然这里边有两个难点。
第一个难点是我们怎么在redis中去快速校验一人一单，还有库存判断
第二个难点是由于我们校验和tomct下单是两个线程，那么我们如何知道到底哪个单他最后是否成功，或者是下单完成，为了完成这件事我们在redis操作完之后，我们会将一些信息返回给前端，同时也会把这些信息丢到异步queue中去，后续操作中，可以通过这个id来查询我们tomcat中的下单逻辑是否完成了。
![image.png](https://cdn.nlark.com/yuque/0/2023/png/32600948/1682819540612-a051af1e-ae5c-42b6-83ef-827dae8f3ae1.png#averageHue=%23f9f5f5&clientId=u9fd07684-81ff-4&from=paste&height=718&id=u165853be&originHeight=718&originWidth=1511&originalType=binary&ratio=1&rotation=0&showTitle=false&size=216625&status=done&style=none&taskId=u3b7c2cda-d5a3-4f6e-be56-1a8a3129f78&title=&width=1511)
现在来看看整体思路：当用户下单之后，判断库存是否充足只需要到redis中去根据key找对应的value是否大于0即可，如果不充足，则直接结束，如果充足，继续在redis中判断用户是否可以下单，如果set集合中没有这条数据，说明他可以下单，如果set集合中没有这条记录，则将userId和优惠卷存入到redis中，并且返回0，整个过程需要保证是原子性的，可以使用lua来操作。
当以上判断逻辑走完之后，我们可以判断当前redis中返回的结果是否是0 ，如果是0，则表示可以下单，则将之前说的信息存入到到queue中去，然后返回，然后再来个线程异步的下单，前端可以通过返回的订单id来判断是否下单成功。
![image.png](https://cdn.nlark.com/yuque/0/2023/png/32600948/1682819648338-7cbebb3c-657b-4f0c-afa9-ab85111f5651.png#averageHue=%23f2eded&clientId=u9fd07684-81ff-4&from=paste&height=760&id=u2e5de251&originHeight=760&originWidth=1606&originalType=binary&ratio=1&rotation=0&showTitle=false&size=240503&status=done&style=none&taskId=u801232b4-0492-4058-a60d-141d02418b6&title=&width=1606)
### 秒杀优化-Redis完成秒杀资格判断
需求：
...
![image.png](https://cdn.nlark.com/yuque/0/2023/png/32600948/1682820095412-3902fe6d-ee8f-4070-a0f8-e0808d6aca2b.png#averageHue=%23f8f6f2&clientId=u9fd07684-81ff-4&from=paste&height=377&id=uaf874896&originHeight=377&originWidth=1128&originalType=binary&ratio=1&rotation=0&showTitle=false&size=81098&status=done&style=none&taskId=u56eb3a42-1f11-42da-b3ff-5af6cc23fca&title=&width=1128)
使用队列的好处在于 **解耦：**所谓解耦，举一个生活中的例子就是：快递员(生产者)把快递放到快递柜里边(Message Queue)去，我们(消费者)从快递柜里边去拿东西，这就是一个异步，如果耦合，那么这个快递员相当于直接把快递交给你，这事固然好，但是万一你不在家，那么快递员就会一直等你，这就浪费了快递员的时间，所以这种思想在我们日常开发中，是非常有必要的。
这种场景在我们秒杀中就变成了：我们下单之后，利用redis去进行校验下单条件，再通过队列把消息发送出去，然后再启动一个线程去消费这个消息，完成解耦，同时也加快我们的响应速度。
这里可以使用一些现成的mq，比如kafka，rabbitmq等等，但是呢，如果没有安装mq，我们也可以直接使用redis提供的mq方案，降低学习、部署的成本。
![image.png](https://cdn.nlark.com/yuque/0/2023/png/32600948/1682820135750-23772e5e-6c94-439f-935c-964541322b70.png#averageHue=%23f8f6f2&clientId=u9fd07684-81ff-4&from=paste&height=377&id=u61e559c2&originHeight=377&originWidth=1128&originalType=binary&ratio=1&rotation=0&showTitle=false&size=81098&status=done&style=none&taskId=ueacee6a8-0cc8-4739-bd6a-6555263059f&title=&width=1128)
### 基于List实现消息队列
**基于List结构模拟消息队列**
消息队列（Message Queue），字面意思就是存放消息的队列。而Redis的list数据结构是一个双向链表，很容易模拟出队列效果。
队列是入口和出口不在一边，因此我们可以利用：LPUSH 结合 RPOP、或者 RPUSH 结合 LPOP来实现。
注意：当队列中没有消息时RPOP或LPOP操作会返回null，并不像JVM的阻塞队列那样会阻塞并等待消息。因此这里应该使用BRPOP或者BLPOP来实现阻塞效果。
![image.png](https://cdn.nlark.com/yuque/0/2023/png/32600948/1682820198335-3c63dba5-aa84-4123-ab24-23454d96dfe0.png#averageHue=%23c3c693&clientId=u9fd07684-81ff-4&from=paste&height=181&id=ubeb980eb&originHeight=181&originWidth=1332&originalType=binary&ratio=1&rotation=0&showTitle=false&size=55909&status=done&style=none&taskId=uf540ed52-c981-4875-8b77-e90e1dff4b8&title=&width=1332)
基于List的消息队列有哪些优缺点？
优点：
...
![image.png](https://cdn.nlark.com/yuque/0/2023/png/32600948/1682820952328-9f7e290f-5f1e-4e15-9e72-8f4b003755a1.png#averageHue=%23fcfafa&clientId=u9fd07684-81ff-4&from=paste&height=451&id=u55293f38&originHeight=451&originWidth=1377&originalType=binary&ratio=1&rotation=0&showTitle=false&size=92491&status=done&style=none&taskId=u234d1518-6c30-4b97-9991-a1a172274ee&title=&width=1377)
基于PubSub的消息队列有哪些优缺点？
优点：
...
发送消息的命令：
![image.png](https://cdn.nlark.com/yuque/0/2023/png/32600948/1682821138629-269dbfd6-742c-4411-afa9-99bc721a419b.png#averageHue=%23122d3e&clientId=u9fd07684-81ff-4&from=paste&height=207&id=uec450509&originHeight=207&originWidth=1507&originalType=binary&ratio=1&rotation=0&showTitle=false&size=230016&status=done&style=none&taskId=ua81bbe70-d105-4549-a915-cffa8d5f31b&title=&width=1507)
例如：
![image.png](https://cdn.nlark.com/yuque/0/2023/png/32600948/1682821144676-9798c47c-5777-4947-bc4e-417a37ed414f.png#averageHue=%23052033&clientId=u9fd07684-81ff-4&from=paste&height=189&id=u2cd0b35c&originHeight=189&originWidth=1508&originalType=binary&ratio=1&rotation=0&showTitle=false&size=105190&status=done&style=none&taskId=ufe7ca398-7521-48af-9248-3cfafa59235&title=&width=1508)
读取消息的方式之一：XREAD
![image.png](https://cdn.nlark.com/yuque/0/2023/png/32600948/1682821148885-bc01ced6-c9cb-40c2-8128-e73747f5e794.png#averageHue=%23132b3f&clientId=u9fd07684-81ff-4&from=paste&height=296&id=u8ac3dcf9&originHeight=296&originWidth=1418&originalType=binary&ratio=1&rotation=0&showTitle=false&size=296114&status=done&style=none&taskId=u39e23930-396e-44df-bd9a-e491dc2bf3f&title=&width=1418)
例如，使用XREAD读取第一个消息：
![image.png](https://cdn.nlark.com/yuque/0/2023/png/32600948/1682821159292-f0b805ae-e88d-4026-8367-264e78b3c50f.png#averageHue=%23021e31&clientId=u9fd07684-81ff-4&from=paste&height=250&id=u84449c4c&originHeight=250&originWidth=1093&originalType=binary&ratio=1&rotation=0&showTitle=false&size=19714&status=done&style=none&taskId=u47b3b711-954f-49cc-b51d-ba13566046a&title=&width=1093)
XREAD阻塞方式，读取最新的消息：
![image.png](https://cdn.nlark.com/yuque/0/2023/png/32600948/1682821166313-7e4fbc69-d65e-4444-8b37-7a92a9b503da.png#averageHue=%23031e32&clientId=u9fd07684-81ff-4&from=paste&height=106&id=u0ce6a5aa&originHeight=106&originWidth=978&originalType=binary&ratio=1&rotation=0&showTitle=false&size=10247&status=done&style=none&taskId=ub3973d84-a113-45a5-893d-590c26a9f0a&title=&width=978)
在业务开发中，我们可以循环的调用XREAD阻塞方式来查询最新消息，从而实现持续监听队列的效果，伪代码如下
![image.png](https://cdn.nlark.com/yuque/0/2023/png/32600948/1682821197846-c52a03c6-40ff-4d43-b4df-c67e8dc32d68.png#averageHue=%23f4f4f3&clientId=u9fd07684-81ff-4&from=paste&height=308&id=ub3aa65f9&originHeight=308&originWidth=844&originalType=binary&ratio=1&rotation=0&showTitle=false&size=29419&status=done&style=none&taskId=uf887de2c-680e-4af2-a43a-db6cf21e132&title=&width=844)
...
![image.png](https://cdn.nlark.com/yuque/0/2023/png/32600948/1682821346407-323fe4d7-af1d-4057-a2f4-bf8380909103.png#averageHue=%23f9f8f7&clientId=u9fd07684-81ff-4&from=paste&height=327&id=u69fed7cd&originHeight=327&originWidth=979&originalType=binary&ratio=1&rotation=0&showTitle=false&size=67847&status=done&style=none&taskId=ube3829ac-5de3-4a11-b603-e6d317b9dc2&title=&width=979)
创建消费者组：
![image.png](https://cdn.nlark.com/yuque/0/2023/png/32600948/1682821394265-e8210608-1386-43a0-ab71-9e0a786ef0f8.png#averageHue=%23052134&clientId=u9fd07684-81ff-4&from=paste&height=41&id=u7ed62dfe&originHeight=41&originWidth=937&originalType=binary&ratio=1&rotation=0&showTitle=false&size=3705&status=done&style=none&taskId=ufc0637e2-c203-4375-ba48-ecf3f294fe7&title=&width=937)
key：队列名称
groupName：消费者组名称
...
消费者监听消息的基本思路：
![image.png](https://cdn.nlark.com/yuque/0/2023/png/32600948/1682821451105-14631ab3-6bd4-4a42-b925-491e7dd0702b.png#averageHue=%23f4f4f4&clientId=u9fd07684-81ff-4&from=paste&height=736&id=u471df79e&originHeight=736&originWidth=808&originalType=binary&ratio=1&rotation=0&showTitle=false&size=68768&status=done&style=none&taskId=u0100eed8-1786-4492-8794-2fff40eab2e&title=&width=808)
...
最后来个小对比
![image.png](https://cdn.nlark.com/yuque/0/2023/png/32600948/1682821459182-703ec8ec-9da5-419d-b84b-2411acc40b19.png#averageHue=%23d9c4c3&clientId=u9fd07684-81ff-4&from=paste&height=426&id=u98374c29&originHeight=426&originWidth=968&originalType=binary&ratio=1&rotation=0&showTitle=false&size=48686&status=done&style=none&taskId=u7396e1b9-b7e8-4ec9-9c36-4271d7d1a82&title=&width=968)
...
修改lua表达式,新增3.6
![image.png](https://cdn.nlark.com/yuque/0/2023/png/32600948/1682821499880-278105e6-cf3a-444a-8c42-f3b827520994.png#averageHue=%23ecf2e7&clientId=u9fd07684-81ff-4&from=paste&height=144&id=ucfca545b&originHeight=144&originWidth=1236&originalType=binary&ratio=1&rotation=0&showTitle=false&size=126798&status=done&style=none&taskId=u70963dc8-bdf9-4075-b877-c064599d29c&title=&width=1236)
```java
private class VoucherOrderHandler implements Runnable {
...
**具体发布流程**
![image.png](https://cdn.nlark.com/yuque/0/2023/png/32600948/1682821537041-0195c7d9-cf70-40cc-be0b-eb7d0d597979.png#averageHue=%23f1edeb&clientId=u9fd07684-81ff-4&from=paste&height=680&id=u30708d22&originHeight=680&originWidth=1567&originalType=binary&ratio=1&rotation=0&showTitle=false&size=465595&status=done&style=none&taskId=u4b036cae-0678-4095-8799-9d7f53672bf&title=&width=1567)
上传接口
```java
@Slf4j
@RestController
...
### 达人探店-查看探店笔记
实现查看发布探店笔记的接口
![image.png](https://cdn.nlark.com/yuque/0/2023/png/32600948/1682821643242-7ffaa1cd-c0f6-4e2a-80a2-63d16c59b8c3.png#averageHue=%23ece4e1&clientId=u9fd07684-81ff-4&from=paste&height=462&id=ub35a1d75&originHeight=462&originWidth=967&originalType=binary&ratio=1&rotation=0&showTitle=false&size=179166&status=done&style=none&taskId=u2fd093b2-1433-4479-9a1f-a03b724a4e0&title=&width=967)
实现代码：
```java
@Override
...
![image.png](https://cdn.nlark.com/yuque/0/2023/png/32600948/1682821671814-bc58f62b-cb37-420f-bc0f-ec602b264883.png#averageHue=%23d8d0c8&clientId=u9fd07684-81ff-4&from=paste&height=707&id=uc9eb6a43&originHeight=707&originWidth=1573&originalType=binary&ratio=1&rotation=0&showTitle=false&size=834523&status=done&style=none&taskId=u7a5c89d3-7d40-4316-b31a-f5e4c14143f&title=&width=1573)
完善点赞功能
需求：
...
为什么采用set集合：
因为我们的数据是不能重复的，当用户操作过之后，无论他怎么操作，都是无效的。
具体步骤：

1. 在Blog添加一个字段
```java
@TableField(exist = false)	//表示数据库并不真实存在该字段
...
public Result likeBlog(Long id) {
    ...
    if (score == null) {
        ...
        if (isSuccess) {
            //3.2保存用户到Redis的SortedSet集合中，将时间戳作为分数值排序
...
![image.png](https://cdn.nlark.com/yuque/0/2023/png/32600948/1682821815150-d06fd931-0312-470d-a0b9-c935c562b132.png#averageHue=%23e4dddc&clientId=u9fd07684-81ff-4&from=paste&height=633&id=u1f190ec1&originHeight=633&originWidth=1306&originalType=binary&ratio=1&rotation=0&showTitle=false&size=421022&status=done&style=none&taskId=u0a1774fd-2702-4bda-b848-8c8e9d049be&title=&width=1306)
接下来对比一下这些集合的区别是什么
所有点赞的人，需要是唯一的，所以我们应当使用set或者是sortedSet
其次我们需要排序，就可以直接锁定使用sortedSet啦
![image.png](https://cdn.nlark.com/yuque/0/2023/png/32600948/1682821829164-0363aa3f-3c34-4f9d-89b0-5152414c436b.png#averageHue=%23c9c9c6&clientId=u9fd07684-81ff-4&from=paste&height=412&id=ueffa1cff&originHeight=412&originWidth=1167&originalType=binary&ratio=1&rotation=0&showTitle=false&size=110911&status=done&style=none&taskId=u058d8bf7-1e42-4dd5-8f1d-f9e58156601&title=&width=1167)
修改代码
点赞逻辑代码
```java
   @Override
...
![image.png](https://cdn.nlark.com/yuque/0/2023/png/32600948/1682821940065-3c414cac-f575-41d3-ab90-8634bf4b2d6e.png#averageHue=%23e4dfda&clientId=u9fd07684-81ff-4&from=paste&height=694&id=u3b6d803e&originHeight=694&originWidth=1404&originalType=binary&ratio=1&rotation=0&showTitle=false&size=500095&status=done&style=none&taskId=ub6bd5485-24cb-45e6-989f-b838d3d4549&title=&width=1404)
实现思路：
需求：基于该表数据结构，实现两个接口：
...
![image.png](https://cdn.nlark.com/yuque/0/2023/png/32600948/1682821957819-386b64ba-9a04-4804-a602-0b4f43ade6b5.png#averageHue=%23fafaf9&clientId=u9fd07684-81ff-4&from=paste&height=250&id=uf9c17f94&originHeight=250&originWidth=1200&originalType=binary&ratio=1&rotation=0&showTitle=false&size=36242&status=done&style=none&taskId=uf19bebd1-a0ae-4adb-8eb0-83da91f4a22&title=&width=1200)
...
以上两个功能和共同关注没有什么关系，重点在于共同关注功能。
![image.png](https://cdn.nlark.com/yuque/0/2023/png/32600948/1682822073579-513d5ecf-1fe8-47de-863a-b03235874a0f.png#averageHue=%23e2dcd7&clientId=u9fd07684-81ff-4&from=paste&height=630&id=u473b4c49&originHeight=630&originWidth=1602&originalType=binary&ratio=1&rotation=0&showTitle=false&size=576422&status=done&style=none&taskId=u854fdecc-8997-48c9-860a-74dbb9c7c8a&title=&width=1602)
```java
...
![image.png](https://cdn.nlark.com/yuque/0/2023/png/32600948/1682822251684-3aa4796b-a275-4324-a77c-c1d70bdb9c26.png#averageHue=%23fcfbfb&clientId=u9fd07684-81ff-4&from=paste&height=563&id=u9e8ac68f&originHeight=563&originWidth=1259&originalType=binary&ratio=1&rotation=0&showTitle=false&size=98139&status=done&style=none&taskId=ucdb1bc9d-7e60-4569-9a04-343ae48aee1&title=&width=1259)先来改造当前的关注列表
...
![image.png](https://cdn.nlark.com/yuque/0/2023/png/32600948/1682822312848-a5b6ae82-96f6-4e59-9971-1b929447557b.png#averageHue=%23e6d6d6&clientId=u9fd07684-81ff-4&from=paste&height=175&id=u6b6485c7&originHeight=175&originWidth=1060&originalType=binary&ratio=1&rotation=0&showTitle=false&size=17373&status=done&style=none&taskId=ud7c79795-68d9-41ea-82a0-48fab75aab9&title=&width=1060)
对于新型的Feed流的的效果：不需要我们用户再去推送信息，而是系统分析用户到底想要什么，然后直接把内容推送给用户，从而使用户能够更加的节约时间，不用主动去寻找。
![image.png](https://cdn.nlark.com/yuque/0/2023/png/32600948/1682822319321-1ddcf8a9-e9cf-4a4b-accf-0dbdda9b7ced.png#averageHue=%23e8d9d9&clientId=u9fd07684-81ff-4&from=paste&height=188&id=u2599b194&originHeight=188&originWidth=1062&originalType=binary&ratio=1&rotation=0&showTitle=false&size=17568&status=done&style=none&taskId=ub10a07e4-d140-4dc0-9f0c-d598a81837d&title=&width=1062)
Feed流产品有两种常见模式：
...
本例中的个人页面是基于关注的好友来做Feed流，只需要拿到我们关注用户的信息，然后按照时间排序即可，因此采用Timeline的模式。该模式的实现方案有三种：
...
![image.png](https://cdn.nlark.com/yuque/0/2023/png/32600948/1682822640137-64bbbe84-077e-475a-8c10-a8ba8bc30985.png#averageHue=%23fcfcfb&clientId=u9fd07684-81ff-4&from=paste&height=493&id=u071ea5f2&originHeight=493&originWidth=1116&originalType=binary&ratio=1&rotation=0&showTitle=false&size=146100&status=done&style=none&taskId=ud396d5d9-968f-44fb-9cc0-d9127b1b544&title=&width=1116)
**推模式**：也叫做写扩散。
推模式是没有写邮箱的，当张三写了一个内容，此时会主动的把张三写的内容发送到他的粉丝收件箱中去，假设此时李四再来读取，就不用再去临时拉取了
优点：时效快，不用临时拉取
缺点：内存压力大，假设一个大V写信息，很多人关注他， 就会写很多分数据到粉丝那边去
![image.png](https://cdn.nlark.com/yuque/0/2023/png/32600948/1682822683010-6a3f58d3-1125-4481-a142-38b4905c30e5.png#averageHue=%23fdfdfc&clientId=u9fd07684-81ff-4&from=paste&height=689&id=ue6c02255&originHeight=689&originWidth=998&originalType=binary&ratio=1&rotation=0&showTitle=false&size=160583&status=done&style=none&taskId=ub49dd03a-016f-4b59-b181-3f217143260&title=&width=998)
**推拉结合模式**：也叫做读写混合，兼具推和拉两种模式的优点。
推拉模式是一个折中的方案，站在发件人这一段，如果是个普通的人，那么我们采用写扩散的方式，直接把数据写入到他的粉丝中去，因为普通的人他的粉丝关注量比较小，所以这样做没有压力，如果是大V，那么他是直接将数据先写入到一份到发件箱里边去，然后再直接写一份到活跃粉丝收件箱里边去，现在站在收件人这端来看，如果是活跃粉丝，那么大V和普通的人发的都会直接写入到自己收件箱里边来，而如果是普通的粉丝，由于他们上线不是很频繁，所以等他们上线时，再从发件箱里边去拉信息。
![image.png](https://cdn.nlark.com/yuque/0/2023/png/32600948/1682822715256-aa80e2a8-eca4-4e7a-a38c-100b8e762f9b.png#averageHue=%23fcfbfb&clientId=u9fd07684-81ff-4&from=paste&height=499&id=ub1437aff&originHeight=499&originWidth=1086&originalType=binary&ratio=1&rotation=0&showTitle=false&size=170253&status=done&style=none&taskId=u526daac1-2839-4137-847a-b731a702e95&title=&width=1086)
### 好友关注-推送到粉丝收件箱
需求：
...
![image.png](https://cdn.nlark.com/yuque/0/2023/png/32600948/1682822760910-c141bffd-645e-4003-abcd-edb81561e265.png#averageHue=%23f9f8f7&clientId=u9fd07684-81ff-4&from=paste&height=494&id=u50361e77&originHeight=494&originWidth=1141&originalType=binary&ratio=1&rotation=0&showTitle=false&size=110689&status=done&style=none&taskId=ube8f7f72-eaa3-477f-b013-06fa97c7a22&title=&width=1141)
Feed流的滚动分页
我们需要记录每次操作的最后一条，然后从这个位置开始去读取数据
举个例子：我们从t1时刻开始，拿第一页数据，拿到了10~6，然后记录下当前最后一次拿取的记录，就是6，t2时刻发布了新的记录，此时这个11放到最顶上，但是不会影响我们之前记录的6，此时t3时刻来拿第二页，第二页这个时候拿数据，还是从6后一点的5去拿，就拿到了5-1的记录。我们这个地方可以采用sortedSet来做，可以进行范围查询，并且还可以记录当前获取数据时间戳最小值，就可以实现滚动分页了
![image.png](https://cdn.nlark.com/yuque/0/2023/png/32600948/1682822801443-c6f87840-496b-4dcf-acd5-7de3eefd03f9.png#averageHue=%23f8f7f6&clientId=u9fd07684-81ff-4&from=paste&height=566&id=ua4481d8d&originHeight=566&originWidth=1130&originalType=binary&ratio=1&rotation=0&showTitle=false&size=128972&status=done&style=none&taskId=u77f7c6b7-c8d6-4f64-9bbb-f3001e0bd19&title=&width=1130)核心的意思：就是我们在保存完探店笔记后，获得到当前笔记的粉丝，然后把数据推送到粉丝的redis中去。
```java
@Override
...
![image.png](https://cdn.nlark.com/yuque/0/2023/png/32600948/1682822926118-4c58d190-24ef-45ca-9f24-56c26dd2c031.png#averageHue=%23dfd7d4&clientId=u9fd07684-81ff-4&from=paste&height=624&id=uc209a920&originHeight=624&originWidth=1244&originalType=binary&ratio=1&rotation=0&showTitle=false&size=365225&status=done&style=none&taskId=u7f2657ff-2372-4af2-82e7-8bc05c80fca&title=&width=1244)
一、定义出来具体的返回值实体类
```java
@Data
...
### 附近商户-导入店铺数据到GEO
具体场景说明：
![image.png](https://cdn.nlark.com/yuque/0/2023/png/32600948/1682823046590-718fcb1a-54af-43b2-bbd1-962c6d7966e7.png#averageHue=%23b98a77&clientId=u9fd07684-81ff-4&from=paste&height=493&id=u84e5e464&originHeight=493&originWidth=1200&originalType=binary&ratio=1&rotation=0&showTitle=false&size=362546&status=done&style=none&taskId=ub0658960-736e-4967-b403-5caa9680aae&title=&width=1200)
当我们点击美食之后，会出现一系列的商家，商家中可以按照多种排序方式，我们此时关注的是距离，这个地方就需要使用到我们的GEO，向后台传入当前app收集的地址(我们此处是写死的) ，以当前坐标作为圆心，同时绑定相同的店家类型type，以及分页信息，把这几个条件传入后台，后台查询出对应的数据再返回。
![image.png](https://cdn.nlark.com/yuque/0/2023/png/32600948/1682823055408-43d759f1-c1a6-43ba-b7b9-edf6c5100f0a.png#averageHue=%23d4bfbe&clientId=u9fd07684-81ff-4&from=paste&height=495&id=uf0505850&originHeight=495&originWidth=944&originalType=binary&ratio=1&rotation=0&showTitle=false&size=158396&status=done&style=none&taskId=uf78475d0-1a8e-4185-bb35-839129560c7&title=&width=944)
...
![image.png](https://cdn.nlark.com/yuque/0/2023/png/32600948/1682823256387-a5e4b635-1ced-4175-a6ad-194090050e05.png#averageHue=%23f9f8f7&clientId=u9fd07684-81ff-4&from=paste&height=269&id=u26a91547&originHeight=269&originWidth=879&originalType=binary&ratio=1&rotation=0&showTitle=false&size=39117&status=done&style=none&taskId=u347f0089-3a92-476d-9a03-091bcdf2465&title=&width=879)
...
![image.png](https://cdn.nlark.com/yuque/0/2023/png/32600948/1682823286305-1f9502d3-9fe7-482f-a7fc-90d518f6c381.png#averageHue=%23f8f6f6&clientId=u9fd07684-81ff-4&from=paste&height=267&id=u2ec74046&originHeight=267&originWidth=1215&originalType=binary&ratio=1&rotation=0&showTitle=false&size=36311&status=done&style=none&taskId=u3a2e87c0-c41b-4c6e-97a3-39b973661fd&title=&width=1215)
BitMap的操作命令有：
...
![image.png](https://cdn.nlark.com/yuque/0/2023/png/32600948/1682823321768-b7105ca9-c613-43f3-b7c8-345d84ea0f74.png#averageHue=%23e9e2e2&clientId=u9fd07684-81ff-4&from=paste&height=590&id=ua435fabd&originHeight=590&originWidth=1184&originalType=binary&ratio=1&rotation=0&showTitle=false&size=191437&status=done&style=none&taskId=u586f4916-452c-4451-902d-66fd54fd8c1&title=&width=1184)
**代码**
```java
 @PostMapping("/sign")
 public Result sign(){
...
![image.png](https://cdn.nlark.com/yuque/0/2023/png/32600948/1682823455969-2fae2835-7293-4ced-96d9-72258078da57.png#averageHue=%23f9f9f9&clientId=u9fd07684-81ff-4&from=paste&height=98&id=u36167a6f&originHeight=98&originWidth=791&originalType=binary&ratio=1&rotation=0&showTitle=false&size=14473&status=done&style=none&taskId=u242d8bde-3903-4cdc-a79e-5b3c25c6b99&title=&width=791)
...
![image.png](https://cdn.nlark.com/yuque/0/2023/png/32600948/1682823510551-087d36cc-fced-4e60-8fd3-669a53b7d8b4.png#averageHue=%23cbb4b2&clientId=u9fd07684-81ff-4&from=paste&height=279&id=ub903140e&originHeight=279&originWidth=665&originalType=binary&ratio=1&rotation=0&showTitle=false&size=46383&status=done&style=none&taskId=u50a46011-d44f-4185-8f46-dc76b0f3fa6&title=&width=665)
代码
```java
...
![image.png](https://cdn.nlark.com/yuque/0/2023/png/32600948/1682823627997-239ec373-b0c8-4c6b-911d-381f89a95197.png#averageHue=%23f8f8f8&clientId=u9fd07684-81ff-4&from=paste&height=326&id=u5f8bbe72&originHeight=326&originWidth=1285&originalType=binary&ratio=1&rotation=0&showTitle=false&size=40246&status=done&style=none&taskId=ue62cc75f-478d-47ed-a10c-985637ec92e&title=&width=1285)
现在的问题是这个主键其实并没有那么短，而是很长的一个 主键
哪怕你单独去提取这个主键，但是在11年左右，淘宝的商品总量就已经超过10亿个
所以如果采用以上方案，这个list也会很大，所以我们可以使用bitmap来减少list的存储空间
我们可以把list数据抽象成一个非常大的bitmap，我们不再使用list，而是将db中的id数据利用哈希思想，比如：
id % bitmap.size  = 算出当前这个id对应应该落在bitmap的哪个索引上，然后将这个值从0变成1，然后当用户来查询数据时，此时已经没有了list，让用户用他查询的id去用相同的哈希算法， 算出来当前这个id应当落在bitmap的哪一位，然后判断这一位是0，还是1，如果是0则表明这一位上的数据一定不存在，  采用这种方式来处理，需要重点考虑一个事情，就是误差率，所谓的误差率就是指当发生哈希冲突的时候，产生的误差。
![image.png](https://cdn.nlark.com/yuque/0/2023/png/32600948/1682823657457-b2418bd4-9d20-4249-a1ea-f060dcd57480.png#averageHue=%23f8f7f7&clientId=u9fd07684-81ff-4&from=paste&height=549&id=u212b511b&originHeight=549&originWidth=1302&originalType=binary&ratio=1&rotation=0&showTitle=false&size=58502&status=done&style=none&taskId=u89c919a2-74c7-49bf-b773-1efe6dfc64c&title=&width=1302)
## UV统计
### HyperLogLog
首先我们搞懂两个概念：
...
Hyperloglog(HLL)是从Loglog算法派生的概率算法，用于确定非常大的集合的基数，而不需要存储其所有值。相关算法原理可以参考：[https://juejin.cn/post/6844903785744056333#heading-0](https://juejin.cn/post/6844903785744056333#heading-0)
Redis中的HLL是基于string结构实现的，单个HLL的内存**永远小于16kb**，**内存占用低**的令人发指！作为代价，其测量结果是概率性的，**有小于0.81％的误差**。不过对于UV统计来说，这完全可以忽略。
![image.png](https://cdn.nlark.com/yuque/0/2023/png/32600948/1682823714159-7fd7a7b1-3de3-4b58-a699-61a279c658b0.png#averageHue=%23021e31&clientId=u9fd07684-81ff-4&from=paste&height=401&id=u10265e12&originHeight=401&originWidth=1153&originalType=binary&ratio=1&rotation=0&showTitle=false&size=40552&status=done&style=none&taskId=u59cb6a92-9d83-4202-8a8e-786f286afac&title=&width=1153)
### 测试百万数据的统计
